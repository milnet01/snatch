...
- ✅ [SNAT-0066] **Python tooling from PyPI is pinned by version, never by hash.**
  ...
  Resolved (2026-09-25): requirements-build.txt (requirements.txt +
  PyInstaller) and requirements-ci.txt (ruff, pytest, zizmor) are
  universal hash locks from uv, regenerated by
  scripts/lock-requirements.sh (--check reports a stale lock). Every CI
  install uses --require-hashes; the unpinned pip self-upgrade is gone.
  Both locks installed into a fresh 3.12 venv with --require-hashes
  --only-binary :all:. Full local CI (static-checks + build-linux under
  act) passed. Windows/macOS jobs verify on push.

...
- **Roadmap:** SNAT-0057 (this record).
  Open items this record names: none
...
Python packages from PyPI. Those are hash-locked by a different mechanism —
`requirements-build.txt` and `requirements-ci.txt`, installed with
`--require-hashes` and regenerated by `scripts/lock-requirements.sh`
(SNAT-0066). The scope of this decision is what `scripts/fetch-binaries.sh`
and `scripts/build-linux.sh` fetch; § Known exceptions has the rest.
...
**1. `SNATCH_APPIMAGETOOL` reaches `chmod +x` with no digest compared.** Setting

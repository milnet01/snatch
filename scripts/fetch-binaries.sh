...
case "$uname_s" in
    Linux)
        ...
        ytdlp_asset="yt-dlp"
        case "$uname_m" in
            x86_64|amd64) ffmpeg_slug="linux-x64"   ;;
            aarch64|arm64) ffmpeg_slug="linux-arm64" ;;
    ...
    Darwin)
        ...
        case "$uname_m" in
            arm64) ffmpeg_slug="darwin-arm64" ;;
            x86_64) ffmpeg_slug="darwin-x64"  ;;
...
if [ "$exe_suffix" = "" ] || [ "$uname_s" != "Linux" ]; then
    for stem in yt-dlp ffmpeg ffprobe; do
